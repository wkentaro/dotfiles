...
- **Favor top-to-bottom readability over ravioli code** — A function the reader can follow straight down beats the same logic shredded into a chain of single-caller helpers ("ravioli code", the over-decomposition counterpart to spaghetti). Every tiny `_helper` called once is a jump: the reader leaves the flow, hunts the definition, and reassembles the sequence in their head — indirection that usually costs more than the name saves. This is the aggregate of the rule above: even when one extraction is individually defensible, a function or module made *mostly* of one-caller helpers has been fragmented, not modularized. Keep the cohesive operation in one place, let it read as a sequence of steps, and inline the blocks that exist only to be "small"; extract only the pieces that earn it through reuse, real complexity, scope-narrowing, or flattened nesting. If you reach for a `# section` comment inside a long function, that is the signal to extract — not "this block could be a function."
- **Re-audit helpers after every simplification** — A refactor isn't done when the tests pass; it's done when every helper it touched has been re-justified. Extractions earn their keep from context: number of callers, shared state, nesting flattened, meaning the name adds. Inlining the other callers or collapsing that shared state silently turns a once-justified helper into dead indirection. So after deleting or merging code, sweep the neighbors and inline any wrapper the change just reduced to a single trivial caller — don't leave fossils from the pre-refactor structure.
