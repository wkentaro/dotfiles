...
- When you post any comment on my behalf to a tracker, issue, PR, or MR, start it with a blockquote AI disclaimer (e.g. `> *This was generated by AI.*`). For PR/MR processing verdicts use `> *This was generated by AI during PR processing.*`
...
- When you create an artifact that's not meant to be committed (scratch analysis, draft patches, research notes, intermediate output), put it under `./log/agents/<YYYY-MM-DD>_<task-slug>/` — one directory per task. Reference artifacts by path in your final message so they're findable. This path is gitignored.
